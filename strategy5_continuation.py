...
S5_FIRE_COOLDOWN_SEC     = 1800  # не входить в тот же символ чаще, чем раз в 30 мин
...
class Strategy5Continuation(BaseStrategy):
    ...
    async def _send_close_message(self, trade: dict, exit_price: float, reason: str) -> None:
        ...
        # PnL с учётом структуры выхода S5: TP1 фиксирует 50%, остаток закрывается здесь.
        ...
        logger.debug("S5 close (telegram disabled)", symbol=trade["symbol"],
